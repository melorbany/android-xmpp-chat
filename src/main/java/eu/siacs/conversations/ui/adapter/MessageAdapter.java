        ...
        if (message.getBody() != null) {
            ...
            if (message.getType() != Message.TYPE_PRIVATE) {
                if (message.hasMeCommand()) {
                    ...
                    if (message.getRecMessageContainer() == Message.TYPE_PRIVATE) {

                            String formattedUrl = Message.FILE_DOWNLOAD_URL + formattedBody;
                            formattedUrl.replaceAll("^" + Message.IMG_COMMAND, "");
//                            loadImage(Message.FILE_DOWNLOAD_URL + formattedUrl,viewHolder);

                        final String urlString = Message.FILE_DOWNLOAD_URL + formattedUrl;
                        AsyncTask<Void, Integer, InputStream> task = new AsyncTask<Void, Integer, InputStream>() {
                            ...
                            protected void onPostExecute(InputStream aVoid) {
                                ...
                                if(aVoid !=null){
                                    Bitmap bmp = BitmapFactory.decodeStream(aVoid);
    ...
    private void loadImage(final String urlString,final ViewHolder viewHolder) {
